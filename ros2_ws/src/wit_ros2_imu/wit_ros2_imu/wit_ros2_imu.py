...
class IMUDriverNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('port_name', '/dev/imu')
        ...
        self.get_logger().info(f"Port Name: {port_name}, Baud: {baud}")
        # 创建IMU数据发布器
        ...
        # 启动IMU驱动线程
        self.driver_thread = threading.Thread(target=self.driver_loop, args=(port_name,baud))
        self.driver_thread.start()

    def driver_loop(self, port_name, baud):
        # 打开串口

        try:
            wt_imu = serial.Serial(port=port_name, baudrate=baud, timeout=0.5)
            if wt_imu.isOpen():
